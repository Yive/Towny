...
public class HealthRegenTimerTask extends TownyTimerTask {
	...
	public void incHealth(Player player) {
		...
		if (currentHP < player.getAttribute(Attribute.GENERIC_MAX_HEALTH).getBaseValue()) {
			player.setHealth(Math.min(player.getAttribute(Attribute.GENERIC_MAX_HEALTH).getBaseValue(), ++currentHP));
